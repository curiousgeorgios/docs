---
title: "Set up task dependencies visually"
description: "Pro"
---
...
## Timeline connectors[​](#timeline-connectors "Direct link to Timeline connectors")
...
![Timeline dependencies](/images/core-concepts/issues/issues/timeline-dependencies.webp)
...
* **Finish-to-Start (FS)** Task B can’t start until Task A finishes. This is shown by the relations Blocking (Task A is blocking Task B) or Blocked by (Task B is blocked by Task A). If Task B’s start date is earlier than Task A’s due date, the connecting line will turn red to indicate a conflict.

  ![Finish to start](/images/core-concepts/issues/issues/finish-to-start.webp)
...
* **Start-to-Start (SS)** With this dependency, Task B can’t start until Task A begins, but both tasks can proceed simultaneously from that point onward. This is shown by the relations Starts before (Task A starts before Task B) or Starts after (Task B starts after Task A). If Task B’s start date is earlier than Task A’s start date, the connecting line will turn red to indicate a conflict.
  ...
  ![Start to start](/images/core-concepts/issues/issues/start-to-start.webp)

* **Finish-to-Finish (FF)** Task B cannot be completed until Task A is finished, though both can be worked on simultaneously. This is shown by the relations Finishes Before (Task A finishes before Task B) or Finishes After (Task B finishes after Task A). If Task B’s due date is earlier than Task A’s, the connecting line will turn red to indicate a conflict.

  ![Finish to finish](/images/core-concepts/issues/issues/finish-to-finish.webp)

## Dependency relations[​](#dependency-relations "Direct link to Dependency relations")
...
![Dependency relations](/images/core-concepts/issues/issues/dependency-relations.webp)
